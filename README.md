...
Unificação de Arquivos: Consolidação de diferentes anos usando steps de Append Streams.
Tratamento de Dados Ausentes (Nulos): Remoção de strings de escape e hífens (-) oficiais do INEP, convertendo-os em valores NULL reais no banco de dados para não distorcer os cálculos estatísticos.
Remoção de Duplicados: Utilização de steps de ordenação (Sort Rows) e registros únicos (Unique Rows) para garantir que a dimensão local possua apenas escolas únicas.
